...
function emptyData() {
  return {
    settings: { shopName: "Pokémon GO Shop", createdAt: Date.now(), lastBackupAt: null, pin: "", logoDataUrl: "", receiptBgDataUrl: "" },
    customers: [],
    gameAccounts: [],
    orders: [],
    investmentHistory: [],
...
// resize an uploaded image file down to a small square data-URL (keeps localStorage light)
...
function LockScreen({ pin, shopName, logoDataUrl, onUnlock, onRecover }) {
  const [value, setValue] = useState("");
  const [err, setErr] = useState(false);
  // recover: false (normal) | "intro" (explain + file picker) | "setpin" (choose new PIN after a valid backup was read)
  ...
  return (
    <div className="pgs-root" style={{ alignItems: "center", justifyContent: "center", padding: 24 }}>
      ...
      <form onSubmit={submit} style={{ width: "100%", maxWidth: 280 }}>
        ...
        <button type="button" className="pgs-btn pgs-btn-outline" style={{ width: "100%" }} onClick={() => setRecover("intro")}>ลืม PIN?</button>
      </form>
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <div className="pgs-root">
      ...
      {tab === "settings" && (
        <div className="pgs-scroll" style={{ position: "fixed", inset: "60px 0 74px 0", maxWidth: 430, margin: "0 auto", background: "var(--bg)", zIndex: 15 }}>
          <SettingsTab data={data} setData={setData} onBackup={exportBackup} onRestore={restoreBackup} onExportExcel={exportExcel} onExportPDF={exportPDF} showToast={showToast} back={() => setTab("dashboard")} />
        </div>
      )}
  ...
  return (
    <Modal title={title} onClose={onCancel}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
        ...
        <div style={{ fontSize: 11, color: "var(--muted)", textAlign: "center" }}>ลากรูปหรือใช้ปุ่มลูกศรเพื่อปรับตำแหน่ง แล้วเลื่อนซูมเพื่อเลือกขนาด</div>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 36px)", gap: 4, justifyContent: "center" }}>
          <span />
          <button type="button" className="pgs-iconbtn" onClick={() => nudge(0, 16)}>↑</button>
          <span />
          <button type="button" className="pgs-iconbtn" onClick={() => nudge(16, 0)}>←</button>
          <button type="button" className="pgs-iconbtn" onClick={() => nudge(0, -16)}>↓</button>
          <button type="button" className="pgs-iconbtn" onClick={() => nudge(-16, 0)}>→</button>
        </div>

        <div style={{ width: "100%", display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 11, color: "var(--muted)" }}>ซูม</span>
        ...
        <div
          style={{
            ...
            position: "sticky", bottom: -18, left: 0,
            background: "linear-gradient(180deg, #14151f 0%, var(--bg) 100%)",
            padding: "10px 0 calc(10px + env(safe-area-inset-bottom))", marginTop: 4,
          }}
        >
  ...
  return (
    <Modal title={mode === "add" ? "เพิ่มออเดอร์" : `แก้ไขออเดอร์ ${form.code || ""}`} onClose={onClose}>
      ...
      <div className="pgs-field">
        ...
        <ProofImagePicker value={form.proofImageDataUrl} onChange={(v) => set("proofImageDataUrl", v)} />
      </div>
      <div className="pgs-field">
...
function ProofImagePicker({ value, onChange }) {
  ...
  async function handleFile(e) {
    ...
    setBusy(true);
    try {
      const dataUrl = await fileToLogoDataUrl(file, 720, 0.85);
      onChange(dataUrl);
    } catch {
...
// =================== SETTINGS ===================
function SettingsTab({ data, setData, onBackup, onRestore, onExportExcel, onExportPDF, showToast, back }) {
  const fileRef = useRef(null);
  const logoRef = useRef(null);
  const bgRef = useRef(null);
  const [cropSrc, setCropSrc] = useState(null); // object URL currently open in the cropper
  ...
  function savePin() {
    ...
    setData(d => ({ ...d, settings: { ...d.settings, pin: pinForm.next } }));
  ...
  function removePin() {
    ...
    setData(d => ({ ...d, settings: { ...d.settings, pin: "" } }));
    setPinForm({ current: "", next: "", confirm: "" });
