...
function emptyData() {
  return {
    settings: { shopName: "Pokémon GO Shop", createdAt: Date.now(), lastBackupAt: null, pin: "" },
    customers: [],
    gameAccounts: [],
    orders: [],
    investmentHistory: [],
...
function migrateData(parsed) {
  ...
  d.settings = { ...emptyData().settings, logoDataUrl: "", ...(parsed.settings || {}) };
...
function LockScreen({ pin, shopName, logoDataUrl, onUnlock }) {
  ...
  return (
    <div className="pgs-root" style={{ alignItems: "center", justifyContent: "center", padding: 24 }}>
      ...
      <form onSubmit={submit} style={{ width: "100%", maxWidth: 280 }}>
        ...
        <button type="submit" className="pgs-btn pgs-btn-primary" style={{ width: "100%" }}>ปลดล็อก</button>
      </form>
    </div>
  );
}
...
async function downloadReceiptImage(order, data, custName, accName) {
  ...
  const [logoImg, proofImg] = await Promise.all([loadImageAsync(r.logoDataUrl), loadImageAsync(r.proofImageDataUrl)]);
...
function ReceiptModal({ order, data, custName, accName, onClose, onToast }) {
  ...
  return (
    <Modal title="ใบเสร็จ / สรุปออเดอร์" onClose={onClose}>
      <div style={{
        background: "linear-gradient(165deg, #1b1d2a 0%, rgba(20,21,31,0.9) 100%)",
  ...
  return (
    <div>
      ...
      <div className="pgs-card" style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 8 }}>
        ...
        <div style={{ flex: 1, fontSize: 11, color: "var(--muted)" }}>ใช้เป็นโลโก้ที่แสดงในหน้าแรกของแอป และเป็นไอคอนตอนเปิดแอป/เพิ่มลงหน้าจอโฮม</div>
      </div>
      <button className="pgs-btn pgs-btn-outline" style={{ width: "100%", marginBottom: 8, justifyContent: "flex-start" }} disabled={logoBusy} onClick={() => logoRef.current?.click()}>
        <Upload size={15} /> {logoBusy ? "กำลังอัปโหลด..." : "อัปโหลดโลโก้ร้าน"}
      </button>
